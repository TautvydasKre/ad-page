...
import { Modal, TextInput, Text, Button, Group, Paper, ActionIcon } from "@mantine/core";
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetchComments(); // Fetch comments after deleting a comment
        onNewComment(); // to re-render CommentsButton component with updated comment count
      } else {
        setMessage("comment deleted successfuly ");
      }
    } catch (error) {
      setMessage(`Comment deletion failed: ${error.message}`);
    }
  };
